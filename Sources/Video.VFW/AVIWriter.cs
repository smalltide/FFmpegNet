...
// Two Improvements by Ch. Fuchs 2012
// 
// 1. As the AVIWriter can't write files with other ending then .avi, files with other extensions (eg. wmv, ...) 
//    are renamed to eg. filename.wmv.avi ad after closing .avi is removed again.
//
// 2. As the AVIWriter cant overwrite files, in such a case the existing file is deleted before creating the 
//    new .avi-file with the same filename.
//
...
namespace AForge.Video.VFW
{
	...
	public class AVIWriter : IDisposable
	{
        ...
        //new: save filename for restoring if .avi was appended
        private string ofileName = "";
        // new: flag for restoring .avi-Filename
        ...
        public void Close( )
		{
            lock ( sync )
            {
                ...
                // ------------------------ new: remove .avi after closing if it was appended to the filename ---

                if ((fileNameChanged & (ofileName != "")))
                {
                    File.Move((ofileName+".avi"), ofileName);
                    ofileName = "";
                };
